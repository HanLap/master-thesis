...
Svelte is a custom language that is very similar to HTML, JavaScript, and CSS, which is colocated inside a \mintinline{bash}{.svelte} file. In fact, plain HTML syntax is also valid Svelte syntax (\Cref{fig:svelte-basic-html}).
...
Svelte provides a strong system for reactive programming. Variable changes are automatically tracked and triggers UI updates. Therefore, it is possible to use regular assignment operations to mutate variables, which is not possible in frameworks such as React (\Cref{fig:svelte-basic-reactivity}).
...
To achieve this, Svelte's compiler replaces assignment operations at compile time with operations that automatically trigger changes of the reactive dependency chain. The increment function in the prior example would be compiled to this statement (\Cref{fig:svelte-compiler-output}).
...
\end{myminted}
\caption{Compiler output for the \mintinline{bash}{increment}-function defined in \Cref{fig:svelte-basic-reactivity}}
...
In the prior example, declarative code was defined that triggers reactive UI updates following mutation of a variable. Svelte also provides syntax to trigger computations following mutation of variables (\Cref{fig:svelte-reactive-variable-syntax}) 
...
\end{myminted}
\caption{Example of a reactive-variable in Svelte. Double will automatically be recalculated when count changes.}
...
\end{myminted}
\s{$}
\caption{Svelte-example that runs statements when \mintinline{bash}{count} changes}
...
UI's also often work with lists of data. To this end, Svelte provides an each-block, which can be used to handle iterable values such as arrays (\Cref{fig:svelte-array-syntax}).
...
<div>hello, {value}!</div>
\end{myminted}
\caption{Basic Svelte component that defines the input \mintinline{bash}{value}.}
\label{fig:svelte-component-basic}
\end{listing}

To define inputs that should be passed into the Component, Svelte repurposes the \mintinline{bash}{export}-keyword. In the prior example \mintinline{svelte}{value} is defined as an input of the Component \mintinline{bash}{MyComponent}. The Component can than be imported in another svelte-file and be used like a regular element (\Cref{fig:svelte-component-usage}).
...
In a prior example we already showed usage of \mintinline{svelte}{on:click}. This directive is used to listen to click events. Svelte makes it possible to listen for arbitrary events on an element with the \mintinline{html}{on}-keyword (\Cref{fig:svelte-on-directive}). Furthermore, Svelte provides an event dispatcher to enable components to send their own custom events (\Cref{fig:svelte-custom-event}).
...
\end{myminted}
\caption{Usage of the \mintinline{bash}{on:}-directive to listen to \mintinline{bash}{pointermove} events.}
...
The \mintinline{svelte}{bind:}-directive can be used to create two-way bindings between parent and child elements. In Svelte, data usually flows from parent to child with the child having no way of directly mutating the data that it is being passed. By passing a property using the \mintinline{svelte}{bind:}-directive, Svelte enables data to flow the other way:
 (\Cref{fig:svelte-bind-directive}).
...
\end{myminted}
\caption{Two-way binding with the \mintinline{bash}{bind:}-directive.}
...
<div class:active />
\end{myminted}
\caption{Example usage of the \mintinline{bash}{class:}-directive.}
\label{fig:svelte-class-directive}
\end{listing}

Similar to the previous directive, the \mintinline{svelte}{style:}-directive provides a utility for dynamically setting CSS-styles on an element. The directive takes a value that is used to set the style attribute, where \mintinline{bash}{undefined} and \mintinline{bash}{null} means that the attribute should not be set. In cases where a variable is used to set a style attribute of the same name, Svelte also provides a shorthand (\Cref{fig:svelte-style-directive}).
...
\end{myminted}
\caption{Example usage of the \mintinline{bash}{style:}-directive.}
\label{fig:svelte-style-directive}
\end{listing}

The \mintinline{bash}{use:}-directive is used to register an action on an HTML-element. An action is a function that is called when the element is created. The function receives a reference to the element and can thus be used to augment behavior of an element. In \Cref{fig:svelte-use-directive} the \mintinline{bash}{use:}-directive is used to register event listeners that log to the console when a mouse enters the element. While this functionality can be easily implemented using the \mintinline{bash}{on:}-directive, actions become especially useful when more complex behavior has to be configured in a reusable way. \Cref{sec:sveltekit-form-actions} will introduce an action that progressively enhances form-elements.
...
However, TypeScript currently has technical restrictions. It is not possible to use TypeScript syntax outside the script-block \cite{noauthor_proposal_nodate}. One might for example want to type the parameters of an inline event handler (\Cref{fig:svelte-ts-inline-error}). But, in most cases this can reasonably be fixed by refactoring the inline function to a function definition in the script-block.
\begin{listing}[H]
\begin{myminted}{svelte}{}
...
In this example two new layout groups are introduced (\mintinline{bash}|(restricted)|,\mintinline{bash}|(public)|). Both groups can provide their own layout without having an influence on the URL. For example, the sign-in page is available on \mintinline{bash}{/signin} (not \mintinline{bash}{/(public)/signin}).

\subsubsection{Loading Data}
\label{sec:sveltekit-loading}

SvelteKit facilitates a standard way for fetching dynamic content that is to be rendered on a page. This is required to enable server-side Rendering and static-site generation. To this end, every page can have a dedicated load function that is defined in a \mintinline{bash}|+page.js|-file. When a user requests a page, this load function is executed and the returned data is passed to the \mintinline{bash}{+page.svelte}-file as special property \mintinline{bash}{data} (\Cref{fig:sveltekit-loading-data}).
...
Some data fetching can only take place on the server-side, for example database access. Therefore, SvelteKit also provides an alternative form of load function that is only executed on the server. These server load function are placed in \mintinline{bash}{+page.server.js}-files (\Cref{fig:sveltekit-server-load-function}).
...
The previous section discussed how SvelteKit applications send dynamic data to the client. But, many applications also need a way to send data to the server. To this end, SvelteKit provides form actions. \mintinline{bash}|+page.server.js|-files can export actions which make it possible to post data to the server using HTML's \mintinline{svelte}{<form>}-elements (\Cref{fig:sveltekit-server-action}).
...
SvelteKit provides the functionality to define the rendering strategy for each route independently. By exporting the constants \mintinline{bash}|ssr|, \mintinline{bash}|csr| and \mintinline{bash}{prerender} in a page or a layout, the rendering behavior can be precisely changed (\Cref{fig:sveltekit-rendering-config}).
