...
Like most web frameworks, Svelte uses CSS to style content. To this end, it is possible to use style-tags to define CSS similar to HTML (\Cref{fig:svelte-style-tag}). Svelte automatically scopes defined styles. This means, the style will only affect markup that is in the same file as the style. This can prevent unexpected styling behavior in growing code bases. 
